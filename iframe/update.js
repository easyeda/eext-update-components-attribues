...
document.addEventListener('DOMContentLoaded', async () => {
	...
	try {
		...
		console.log(data);
		let html = '<option value="" disabled selected>请选择原理图</option>';
		let hasSchematic = false;

		for (const item of data) {
			if (item?.schematic?.name) {
				html += `<option value="${item.schematic.name}">${item.schematic.name}</option>`;
				hasSchematic = true;
			}
		}

		if (!hasSchematic) {
			html = '<option value="" disabled selected>无可用原理图</option>';
		}
